...
#read file data
nums <- scan("data/sample_num.txt") # 주어진 파일이 갖고 있는 값들을 벡터로 가져옴. 수치 데이터(ex- 센서데이터)를 읽어오는데 특화됨. 분리문자 : 공백
...
df2 <- read.table("data/product_click.log", stringsAsFactors = T) # 공백이나 탭으로 구분된 경우, read.table
str(df2)
head(df2)
summary(df2$V2)  # 가장 많이 클릭된 상품 확인가능

# 함수 정의와 활용

func1 <- function() {      # 함수 header 부분
  xx <- 10   # 지역변수    # 함수 body 부분
  yy <- 20
  return(xx*yy)
}
func1()
...
func4() # 에러 발생 : 기본값이 없는 인수 "z"가 누락되어 있습니다
func4(10,20,30)
func4(x=1,y=2,z=3)
