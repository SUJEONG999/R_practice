install.packages("RSelenium")  # 동적 스크래핑(크롤링)
library(RSelenium)

remDr <- remoteDriver(remoteServerAddr = "localhost" , port = 4445, browserName = "chrome") # 셀리늄 서버를 접속하는 역할
remDr$open() # Selenium 서버에 의해 제어되는 크롬 브라우저가 기동됨! (브라우저 창이 뜸)

remDr$navigate("http://www.google.com/") # 지정된 url 페이지를 요청하고 랜더링(자바스크립트 코드 수행)

webElem <- remDr$findElement(using = "css selector", "[name = 'q']") # 원하는 태그(여기서는 입력상자)에 대한 DOM 객체를 찾아서
webElem$sendKeysToElement(list("PYTHON", key = "enter")) # <input> 태그에 텍스트 입력하고 엔터키 입력을 자동화 하기 / 파이썬에 대한 검색 결과 보여주게끔


remDr$navigate("http://www.naver.com/") # url에 해당하는 웹페이지 랜더링
str(remDr)

webElem <- remDr$findElement(using = "css selector", "#query") # 태그 한 개 찾기(id 태그 줌)
webElem$sendKeysToElement(list("PYTHON", key = "enter")) # 검색 기능을 자동으로 처리해줌.
str(webElem)
# 프로그램으로 자동화 시킴.
...
one$getElementTagName() # 찾아진 태그의 태그명 추출
one$getElementText() # 찾아진 태그의 태그 내용 추출
one$getElementAttribute("style") # 찾아진 태그의 속성명에 대한 값 추출
...
#복수형으로 노드 추출
more<-remDr$findElements(using='css selector','div') # 태그들 찾기 (리스트 객체로 리턴)
sapply(more, function(x) x$getElementTagName()) # 찾아진 태그들의 태그명들을 추출하여 리스트로 리턴
sapply(more, function(x) x$getElementText())  # 찾아진 태그들의 컨텐트들을 추출하여 리스트로 리턴

#복수형으로 없는 추출
more<-remDr$findElements(using='css selector','p') # 없으면 오류가 발생하지 않음 비어있는 리스트 리턴
print(more)  # list() 로 나옴.
if(length(more) == 0) 
  cat("<p> 태그는 없슈\n") # 없으면 메세지 나오게끔 
sapply(more, function(x) x$getElementTagName()) # 꺼낼 데이터가 없으니깐 비어있는 리스트 추출--> list()
sapply(more, function(x) x$getElementText()) # 꺼낼 데이터가 없으니깐 비어있는 리스트 추출---> list()
...
one<-remDr$findElement(using='css selector','a:nth-of-type(4)') # 태그 한개 찾기
one$getElementTagName() # 찾아진 태그의 태그명 추출
one$getElementText() # 찾아진 태그 내용 추출
one$clickElement()  # 찾아진 태그에서 클릭 이벤트 발생시키기 ---- 방법 1


url <- "http://unico2013.dothome.co.kr/crawling/exercise_bs.html"
remDr$navigate(url) # url에 해당하는 웹페이지 랜더링

one<-remDr$findElement(using='css selector','a:nth-of-type(3)') # 태그 한개 찾기
one$getElementTagName() # 찾아진 태그의 태그명 추출
one$getElementText() # 찾아진 태그 내용 추출
remDr$executeScript("arguments[0].click();",list(one)); # 가끔 clickElement() 일 안할 때가 있음. --- 방법 2
# 자바스크립트 코드 # 아규먼트를 클릭이벤트 발생
...
more$getElementTagName()  # 찾아진 태그의 태그명 추출
more$getElementText() # 찾아진 태그 내용 추출

more$clickElement() # 찾아진 태그에서 클릭 이벤트 발생시키기 ---- 방법 1 사용


# 2페이지부터 10페이지까지 링크 클릭하여 페이지 이동하기 
for (i in 4:12) { # 크롬 개발자도구를 통해 확인해보면 2페이지가 a:nth-child(4) 해당함.
  nextCss <- paste0("#cbox_module>div>div.u_cbox_paginate>div> a:nth-child(",i,") > span")  # 몇번째 자식 태그인가
  nextPage<-remDr$findElement(using='css selector',nextCss) # css 선택자 태그 찾기
  nextPage$clickElement() # 찾아진 태그에서 클릭 이벤트 발생시키기
  Sys.sleep(2)
}
...
remDr$navigate(url)
#베스트 댓글 내용 읽어오기
bestReviewNodes<-remDr$findElements(using ="css selector","ul.u_cbox_list span.u_cbox_contents") # 태그들을 찾기
sapply(bestReviewNodes,function(x){x$getElementText()}) # 찾아진 태그들의 컨텐트들을 추출하여 리스트로 리턴
...
totalReviewNodes<-remDr$findElements(using ="css selector","ul.u_cbox_list span.u_cbox_contents") # 태그들을 찾기
sapply(totalReviewNodes,function(x){x$getElementText()}) # 찾아진 태그들의 컨텐트들을 추출하여 리스트로 리턴
...
repl_v <- c(repl_v, unlist(vest_repl)) # 벡터로 만들어줌
...
repl_v <- c(repl_v, unlist(repl)) # 기존 벡터에 합치기
...
repeat { 
  ...
  if(length(nextPage) == 0)  break; # 다음 페이지 없으면, break
...
repeat{
  ...
  remDr$executeScript("arguments[0].click();",pageLink) # 가끔 clickElement() 일 안할 때가 있음. --- 방법 2
  Sys.sleep(2)
...
for (booksite in booksites) {
  ...
  bookthumburl <- sapply(bookthumbenodes, function(x) {x$getElementAttribute("src")}) # 이미지의 url 받아옴
  ...
  if (!dir.exists('output/book')) # 북이라는 폴더 있는지 확인
...
# [ YES24의 명견만리 댓글 읽어오기 ]--- 스크롤 효과, 짧은 댓글을 긴 댓글로 바꿔서!
...
remDr$executeScript("scrollTo(0, 0)", args = list(webElem))  # 페이지를 아래로 내리는(스크롤) 효과
Sys.sleep(1)
remDr$executeScript("scrollBy(0, 3200)", args = list(webElem)) # 현재 위치로부터 상대적으로 더 내려가라(움직여라)
Sys.sleep(1)
...
repeat {
  for(index in 3:7) {  # 한페이지당 리뷰글 5개!
    fullContentLinkcss <- paste("#infoset_reviewContentList > div:nth-child(",index,") > div.reviewInfoBot.crop > a", sep='') # 전체 댓글
...
repeat{
  ...
  if(index %% 3 == 0 && index != total)
    remDr$executeScript("var dom=document.querySelectorAll('#mCSB_3_container > ul > li')[arguments[0]]; dom.scrollIntoView();", list(index))   # 특정 태그 안에서의 스크롤 처리
}
