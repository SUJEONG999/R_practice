...
bind_rows(group_a, group_b)  # 행 기준으로 합칠 때 사용하는 함수 # rbind와 유사
bind_cols(group_a, group_b)  # 열 기준으로 합칠 때 사용하는 함수 # cbind와 유사

one <- data.frame(c(x = c(1:1000000), y = c(1:1000000))); head(one) 
two <- data.frame(c(x = c(1:1000000), y = c(1:1000000))); head(two)
...
boxplot(mpg$hwy, range=0) # range=0 : 이상치 체크하지 않는다!
summary(mpg$hwy)
#상자그림 통계치 출력
...
str(mpg)
ggplot(mpg)
ggplot(mpg,aes(displ, hwy)) # 축 추가
ggplot(mpg,aes(displ, hwy)) + geom_point() # 그리고 싶은 그림의 함수를 마저 호출함.
...
sketch + geom_point() # 산점도
sketch + geom_line() # 선그래프
sketch + geom_point() + coord_cartesian(xlim=c(3, 6)) # x축의 범위를 조정
sketch + geom_point() + coord_cartesian(xlim=c(3, 6), ylim=c(10, 30)) # x축과 y축의 범위를 조정
sketch + geom_point(aes(color=drv)) # 다른 변수를 가지고 와서 색깔을 입힘
...
ggplot(airquality, aes(x=Day, y=Temp)) + geom_point(size=3, color="red") # 점 크기, 색깔 지정
ggplot(airquality, aes(x=Day, y=Temp)) + geom_line() # 선으로 그리기
ggplot(airquality, aes(x=Day, y=Temp)) + geom_line() + geom_point() # 선과 점으로 표현
...
# 박스그래프
ggplot(airquality, aes(x=Day, y=Temp, group=Day)) + geom_boxplot() # day별로(일자별로) 박스 그래프를 그려줌

# 히스토그램
ggplot(airquality, aes(Temp)) + geom_histogram() # 히스토그램
...
# 막대그래프
ggplot(mtcars, aes(x=cyl)) + geom_bar() # 실린더별 개수
ggplot(mtcars, aes(x=cyl)) + geom_bar(width=0.5) # 막대의 너비 사이즈 조정
ggplot(mtcars, aes(x=cyl)) + geom_bar(aes(fill=factor(gear)), alpha=0.5) # gear를 팩터화해서 색을 채움. 
ggplot(mtcars, aes(x=cyl)) + geom_bar(aes(fill=factor(gear)), alpha=1.0) # alpha 값은 투명도를 의미함. 1.0은 또렷!

# 그래프에 테마 적용
ggplot(mtcars, aes(x=gear))+geom_bar() + labs(x="기어수", y="자동차수", title="변속기 기어별 자동차수") + theme_gray() # 기본 디폴트
ggplot(mtcars, aes(x=gear))+geom_bar() + labs(x="기어수", y="자동차수", title="변속기 기어별 자동차수") + theme_bw() # 테마 블랙화이트
...
ggplot(mtcars, aes(x=gear))+geom_bar() + labs(x="기어수", y="자동차수", title="변속기 기어별 자동차수") + theme_void() # 막대만 나옴


# 변수에 담은 다음에 테마 적용하는 경우
...
bar_data <- classDF[3] # 혈액형만 들어있는 데이터 프레임 생성
bar_data
str(bar_data)
table(bar_data$bloodType)
ggplot(bar_data, aes(x=bloodType)) + geom_bar()
ggplot(classDF, aes(x=bloodType)) + geom_bar(aes(fill=gender)) # 혈액형 막대그래프에 성별을 나눠서 채워라
...
ggplot(coSalesDF, aes(x=year, y=sales)) + geom_line() # 두개씩만 있어서 무엇을 나타내는지 알 수 없음
ggplot(coSalesDF, aes(x=year, y=sales)) + geom_line(aes(group=company)) # 그래서 그룹을 나눠서(회사별로) 그림. 회사별 매출 실적 추이
# 선 색상 및 두께 설정
ggplot(coSalesDF, aes(x=year, y=sales)) + geom_line(size=2, aes(group=company,colour=company)) # colour=company : 회사별로 색깔을 입히게끔!
...
ggplot(coSalesDF, aes(x=year, y=sales)) + geom_line(size=2, aes(group=company,colour=company))+scale_colour_manual(values = c("orange", "green")) # 칼라를 직접 정해주고 싶다면 scale_colour_manual
# 선의 종류 :  0 = blank, 1 = solid, 2 = dashed, 3 = dotted, 4 = dotdash, 5 = longdash, 6 = twodash
ggplot(coSalesDF, aes(x=year, y=sales)) + geom_line(size=2, aes(group=company,colour=company), linetype = 3) # 선의 종류를 점으로
ggplot(coSalesDF, aes(x=year, y=sales)) + geom_line(size=2, aes(group=company,colour=company), linetype = "dotdash") # 명칭으로 줘도됨!
# 점의 종류와 색상
ggplot(coSalesDF, aes(x=year, y=sales)) + geom_line(size=2, aes(group=company, colour=company)) + geom_point(size=2, shape = 5) # 선그래프에 포인트를 줌!
...
ggplot(Cars93, aes(x=Weight, y=MPG.highway, fill=Price)) + geom_point(colour="grey", shape=21, size=6) # 채우기 값을 변수로 줄 수 있음.
...
ggplot(data=w, aes(x=year)) + geom_bar() # 갯수 세다보니.. 한개씩으로 나옴
ggplot(data=w, aes(x=year, y=weight)) + geom_bar() # 갯수를 세서 막대를 그리는게 기본이라.. 에러난다!!
ggplot(data=w, aes(x=year, y=weight)) + geom_col() # x,y 둘다 쓸때는 대신 geom_col로~
ggplot(data=w, aes(x=year, y=weight)) + geom_bar(stat="identity") #혹은 이 방법으로! # stat="identity" :  데이터의 종류를 한 그래프 안에 나눠서 표시해주는 bar 그래프
ggplot(data=w, aes(x=year, y=weight)) + geom_bar(stat="identity") + coord_cartesian(ylim=c(60, 75)) # y축 범위 설정
ggplot(data=w, aes(x=year, y=weight)) + geom_bar(aes(fill=year), stat="identity") + coord_cartesian(ylim=c(60, 75)) # 연도별로 색깔 지정
ggplot(data=w, aes(x=year, y=weight)) + geom_bar(aes(fill=year), colour="blue", stat="identity") + coord_cartesian(ylim=c(60, 75)) # 테두리선 지정
ggplot(data=w, aes(x=year, y=weight)) + geom_bar(aes(fill=year), stat="identity") + geom_label(aes(label=weight))+coord_cartesian(ylim=c(60, 75)) # 라벨링
ggplot(data=w, aes(x=year, y=weight)) + geom_bar(aes(fill=year), stat="identity") + geom_label(aes(label=weight), nudge_y=1)+coord_cartesian(ylim=c(60, 75)) # 라벨링 위치 변경
ggplot(data=w, aes(x=year, y=weight)) + geom_bar(aes(fill=year), stat="identity") + geom_label(aes(label=weight), nudge_y=2)+coord_cartesian(ylim=c(60, 75)) # 라벨링 위치 변경
...
ggplot(mpg, aes(x=displ, y=hwy,  color= manufacturer))+geom_point() # 제조회사별로 칼라지정
ggplot(mpg, aes(x=displ, y=hwy,  color= manufacturer))+geom_point() + scale_color_manual(values = rainbow(15)) # 팔레트 지정
...
# 인터랙티브 그래프 만들기
ggplotly(p) # 정적인 그래프를 동적인 그래프로 바꿔주는 역할
...
# 인터랙티브 막대 그래프 만들기
p <- ggplot(data = diamonds, aes(x = cut, fill = clarity)) + geom_bar(position = "dodge") # position = "dodge": 데이터의 종류를 나눠 따로 표시해주는 그래프
ggplotly(p)
