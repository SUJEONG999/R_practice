...
library(httr)
# //a/b/c : // 맨앞에 누가 있든 간에
# /a//b/c : // a와 b 사이에 중간 생략 가능(너무 긴경우)

# [ 예제8 ]
# W3C의 HTTP 프로토콜 스팩에서 Table of Contents 읽기                        # 개발자도구 : body > div > h2 도 가능
title2 = html_nodes(read_html('http://www.w3.org/Protocols/rfc2616/rfc2616.html'), 'div.toc h2')
title2 = html_text(title2)             # xpath = '/html/body/div/h2' 도 가능,  xpath = 안붙이면 선택자로 인식해서 오류 뜸.(/인식X)
title2
...
article.href = unique(html_attr(link, 'href')) # 같은 URL 은 제외하고 출력
article.href # 링크 URL 을 뽑아옴

# [ 예제9 ]
# 이미지, 첨부파일 다운 받기 (바이너리 문서, 0과 1로만 구성, 특정 프로그램이 있어야만 볼 수 있음. ppt,word 등등 ) 
# cf. 텍스트문서 :txt, csv,html
# pdf
library(httr)
...
imgs = html_nodes(h, 'img') # img 태그 찾기
img.src = html_attr(imgs, 'src') # src 속성 가져오기
for(i in 1:length(img.src)){ 
  res = GET(paste('http://unico2013.dothome.co.kr/',img.src[i], sep="")) # 이미지를 요청
  writeBin(content(res, 'raw'), paste('c:/Temp/', img.src[i], sep="")) # 이미지 저장  # raw : 응답된 형식 그대로
} 
...
busRouteId <- df[df$busRouteNm == 146, "busRouteId"] # 해당되는 고유 번호뽑아냄
busRouteId
...
df <- data.frame(json_obj) # 데이터 프레임으로 변환
df <- df[-1] # 1열 제외
names(df) <- c("className", "unitName", "cycle", "keystatName", "dataValue") # 새로운 변수명 설정
View(df)
options(scipen=100)
View(df)
