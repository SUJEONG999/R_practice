...
text <- read_html(url) # HTML 웹 페이지를 요청해서 받아오기
text

nodes <- html_nodes(text, css = "div") # 태그 선택자 # 모든 div 태그들을 추출.
nodes
title <- html_text(nodes)
title

node1 <- html_nodes(text, "div:nth-of-type(1)") # div 태그 중에 첫 번째 
node1
html_text(node1) # 노드에서 컨텐트 추출하기
html_attr(node1, "style") # style 이라는 속성명의 속성값 추출

node2 <- html_nodes(text, "div:nth-of-type(2)") # div 태그 중에 두 번째 
node2
html_text(node2)
html_attr(node2, "style") # 지정 노드에서 style 이라는 속성명의 속성값 추출
...
# [ 예제2 ]
# 웹문서 읽기
url <- "https://www.data.go.kr/tcs/dss/selectDataSetList.do" # 공공 데이터 포털의 목록과 개요부분
html <- read_html(url)
html

# 목록 아이템 추출
title <- html_text(html_nodes(html, "#apiDataList .title")) # "#apiDataList .title"에서 첫번째는 id 태그이고, 두번째는 class 속성임.
title  # trim = T : 지정하면 앞 뒤에 쓸데없는 분리 문자(공백, 탭, 개행문자) 없애줌. 

# 목록 아이템 설명 추출
desc <- html_text(html_nodes(html, "#apiDataList .ellipsis")) # id 태그 : apiDataList , class 속성 : ellipsis 인 거에서 추출
desc

# 데이터 정제: 제어문자를 공백으로 대체
title <- gsub("[[:cntrl:]]", "", title) # Control charcters
title # 훨씬 깔끔해짐.

desc <- gsub("[[:cntrl:](</br>)∙]", "", desc) # \r\n- 없애기 위해 [:cntrl:], br태그 없애기 위해 (</br>) 
desc
...
review <- html_text(review , trim=TRUE) # 분리 문자 없애기!
review <- review[nchar(review)>0] # 길이가 0인 애들(리뷰 없음) 은 제외!
review
page <- data.frame(title, point, review) # 데이터 프레임은 갯수가 맞아야 만들어짐!
print(page)

# [ 예제4 ]
# 영화 제목, 평점, 리뷰글 읽기
text<- NULL; vtitle<-NULL; vpoint<-NULL; vreview<-NULL; page=NULL  # 리뷰글이 없는 거는 NULL 들어가게끔!
url<- "http://movie.naver.com/movie/point/af/list.nhn"
text <- read_html(url,  encoding="CP949")
text
...
library(XML)
library(rvest)
imsi <- read_html("http://www.hani.co.kr/")    # // : 조상이 몇 개가 있든 간에
t <- htmlParse(imsi)           # * : 태그이름이 뭐든 간에, 해당 id 태그 찾고, 서브태그들을 찾아라.
content<- xpathSApply(t,'//*[@id="main-top01-scroll-in"]/div/div/h4/a', xmlValue); # / : 자식, // : 자손, @ : 속성명 앞에 꼭 붙여야함.
content
