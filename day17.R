...
predict.lm(model, newdata = data.frame(height_father = 170, height_mohter = 160), interval = "confidence") # 하한값, 상한값까지 보여줌


# 어떤 공장에서 나오는 제품의 강도가 그 공정의 온도와 압력에 어떤 영향을 받는가
...
summary(mtcars)
cor(mtcars) # 상관계수가 다소 높음 확인

# car패키지 안에 들어있음. 산점도 그리기
...
mtcars.lm <- lm(mpg ~ hp + wt + disp + drat, data=mtcars) # 방법1
summary(mtcars.lm)

mtcars.lm <- lm(mpg ~ ., data=mtcars) # 방법2
summary(mtcars.lm)
...
step(mtcars.lm, direction="backward") # 후진제거법
step(mtcars.lm, direction="both") 
start.lm <- lm(mpg~1, data=mtcars)
step(start.lm, scope=list(lower=start.lm, upper=mtcars.lm), direction="forward") # 전진선택법
...
mtcars.regsubsets <- regsubsets(x=mpg ~ hp + wt + disp + drat, data=mtcars, nbest=4) # 조합 4가지 찾기

# 위로갈수록 수정된 결정계수가 커지는 조합을 알 수 있음. # 결정계수를 우선적으로 생각해서 만든 조합
...
mtcars.lm3 <- lm(mpg ~ . - disp, data=mtcars) # 남아있는 독립변수 중에서 disp 변수는 제외
AIC(mtcars.lm1, mtcars.lm2, mtcars.lm3) # 3가지 회귀식의 각각 AIC 도출

summary(mtcars.lm1) # AIC 가장 낮은 것으로 최종선택
summary(mtcars.lm2)
summary(mtcars.lm3)
...
summary(multico.lm) # 유의미하지 않은 변수들 보임

# 상관계수 확인
cor(multico[-1])
# vif 확인 (car 패키지)
library(car)
vif(multico.lm) # -- 다중공선성 판단
# x4가 가장 vif 가 가장 높음 -- 먼저 제거
multico.lm <- lm(Y~X1+X2+X3, data=multico)
summary(multico.lm)
vif(multico.lm) # 재확인 : 값이 작아짐(독립변수 간의 강한 상관관계는 안보임)

# 하지만 summary에서 유의미하지 않은 변수들이 있었음.
# 그래서 변수들간의 어떤 조합이 유의미하게 만드는지 확인!
multico.regsubsets <- regsubsets(x=Y ~ . - ID, data=multico, nbest=2) # 단, ID는 제외
...
ggmap(map) + 
  ...
  geom_text(aes(x=mk$lon, y=mk$lat, label="우리가 공부하는 곳", 
                color="red", vjust=0, hjust=0, family = "maple"))
...
df <- data.frame(name=names,
                 lon=gc$lon,
                 lat=gc$lat)
cen <- c(mean(df$lon),mean(df$lat)) # 중요 : 하나의 지도 그래프를 그리려고 중심을 평균으로!
...
station_lonlat
save(station_lonlat, file="output/station_lonlat.rda") # 두 번째 테스트부터는 저장했다가 읽자구요
#load("station_lonlat.rda")

seoul_lonlat = unlist(geocode('seoul', source = 'google')) # 벡터로 만듦
?qmap
...
leaflet() %>%
  ...
  addProviderTiles("NASAGIBS.ViirsEarthAtNight2012") # 밤에 찍은 위성지도
