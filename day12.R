...
barplot(ds, main='인구 추정',           # 막대를 옆쪽으로 붙여서 표시
        col=c('green','blue','yellow'),
        beside=TRUE)


barplot(ds, main='인구 추정',   # 막대 그래프에 범례추가
        col=c('green','blue','yellow'),
        beside=TRUE,
        legend.text=T)

par(mfrow=c(1, 1), mar=c(5, 5, 5, 7)) # 범례를 그래프 밖에 표시
barplot(ds, main='인구 추정',  
        col=c('green','blue','yellow'),
        beside=TRUE,
        legend.text=T,
...
(성적 <- read.table("data/성적.txt", header=TRUE))  # header=TRUE : 설정해줘야함

plot(성적$학번, 성적$국어, main="성적그래프", xlab="학번", ylab="점수",  xlim=c(0, 11), ylim=c(0, 11)) 

ymax <- max(성적[3:5]) #성적 데이터 중에서 최대값을 찾는다(y 축의 크기 제한)
ymax
pcols<- c("red","blue","green") # 칼라 지정
png(filename="output/성적.png", height=400, width=700, bg="white") # 출력을 png파일로 설정 (화면에 출력x)
...
legend(11,30, names(성적1), cex=0.8, fill=rainbow(3)); # 범례 사이즈 조정
